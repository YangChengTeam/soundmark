...
/**
 * Created by caokun on 2019/11/8 13:59.
 */

public class VideoTextureView extends TextureView {

    private String TAG = "VideoPlay_VideoTextureView";
...
//        }
        ...
        if (parentWidth != 0 && parentHeight != 0 && videoWidth != 0 && videoHeight != 0) {
            ...
            if (VideoConstants.VIDEO_DISPLAY_TYPE_ORIGINAL ==
                    VideoPlayerManager.getInstance().getVideoDisplayType()) {
                Log.d(TAG, "onMeasure: 原始大小");
                height = videoHeight;
                width = videoWidth;
                //缩放至控件宽高，会裁剪超出比例的画面
            } else if (VideoConstants.VIDEO_DISPLAY_TYPE_CUT ==
                    VideoPlayerManager.getInstance().getVideoDisplayType()) {
                Log.d(TAG, "onMeasure: 裁剪铺满");
